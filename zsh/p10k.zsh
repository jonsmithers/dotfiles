function prompt_gitmob() {
    ...
    function update_initials() {
      ...
      echo "$initials" > /tmp/git-mob-initials
    }
    silent_background update_initials
    if [[ ! -f /tmp/git-mob-initials ]]; then
      return
    fi
    local initials="$(cat /tmp/git-mob-initials)"
    if [[ -z "$initials" ]]; then
      return
    fi
    p10k segment -i '' -f grey -t "$initials"
  }

# Generated by Powerlevel10k configuration wizard on 2023-01-16 at 17:42 EST.
