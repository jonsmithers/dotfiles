function prompt_gitmob() {
    if ! command -v git-mob-but-faster &> /dev/null; then
      return
    fi
    local initials="$(git mob-print-but-faster --initials)"
    if [[ -z "$initials" ]]; then
      p10k segment -i '' -f lightgrey -t "solo"
      return
    fi
    p10k segment -i '' -f lightgrey -t "$initials"
  }
